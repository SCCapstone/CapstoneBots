"""Make author_id, created_by, and added_by nullable for account deletion

Revision ID: 002
Create Date: 2026-02-25

This migration updates branches.created_by, commits.author_id, and
...
def upgrade():
    # Make branches.created_by nullable and add ON DELETE SET NULL
    op.alter_column('branches', 'created_by', existing_type=postgresql.UUID(as_uuid=True), nullable=True)
    op.drop_constraint('branches_created_by_fkey', 'branches', type_='foreignkey')
    op.create_foreign_key(
        'branches_created_by_fkey', 'branches', 'users',
...
def downgrade():
    # Revert branches.created_by to NOT NULL without ON DELETE SET NULL
    op.drop_constraint('branches_created_by_fkey', 'branches', type_='foreignkey')
    op.create_foreign_key(
        'branches_created_by_fkey', 'branches', 'users',
        ['created_by'], ['user_id']
    )
    op.alter_column('branches', 'created_by', existing_type=postgresql.UUID(as_uuid=True), nullable=False)
