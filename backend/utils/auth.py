...
def create_email_verification_token(email: str) -> str:
    ...
    The token contains a 'purpose' claim to prevent cross-use with other tokens.
    Expires after EMAIL_VERIFY_EXPIRE_HOURS (default 24h).
    """
    now = datetime.now(timezone.utc)
    expire = now + timedelta(hours=EMAIL_VERIFY_EXPIRE_HOURS)
    payload = {
        "sub": email,
...
def decode_email_verification_token(token: str) -> dict:
    """
    Decode an email-verification JWT and verify its purpose claim.
    ...
    try:
        ...
        logger.warning(f"Verification token decode failed: {str(e)}")
        raise

    if payload.get("purpose") != "email-verification":
        raise ValueError("Token is not a valid email-verification token")
